...
function restoreSnapshot(snapshot: HostRow): void {
  db.update(hosts)
    .set({
      ...
      sslCertPath: snapshot.sslCertPath ?? undefined,
      sslKeyPath: snapshot.sslKeyPath ?? undefined,
      hsts: snapshot.hsts,
      ...
      advancedNginx: snapshot.advancedNginx ?? undefined,
      clientMaxBodySize: snapshot.clientMaxBodySize ?? undefined,
...
export async function publishHost(auth: AuthContext, id: number): Promise<HostRow> {
  ...
  db.update(hosts)
    .set({
      ...
      groupId: (effective.groupId as number | null | undefined) ?? undefined,
