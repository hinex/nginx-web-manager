...
describe("previewConfigEdit / applyConfigEdit — stream host files (host-<id>-stream.conf)", () => {
  ...
  function makeStreamHostRow(overrides: Record<string, unknown> = {}) {
    return {
      id: 1,
      groupId: null,
      domains: [],
      ...
      streamPorts: [
        {
          port: 3306,
          protocol: "tcp",
          upstreams: [{ server: "10.0.0.5", port: 3306, weight: 1 }],
          balanceMethod: "round_robin",
        },
      ],
      createdAt: new Date(),
  ...
  // Mirrors renderStream()'s exact header + body assembly (generator.ts's
  // format), using the real buildStreamBlock so the diff-against-baseline
  // technique in the tests below matches how classifyStreamFor builds its
  // "expected" side. dnsResolver/"" and dnsResolverValid/"30s" match the
  // mocked settingsStore defaults (empty settingsStore -> "" / "30s").
  function streamBaselineText(row: any) {
    const streamBlock = buildStreamBlock(row.id, row.streamPorts, "", "30s");
  ...
  it("recognizes host-<id>-stream.conf as a managed stream host file (not null, not the HTTP path)", () => {
    const row = makeStreamHostRow();
    hostsStore.set(1, row);
    const baseline = streamBaselineText(row);

    const preview = previewConfigEdit(auth, "conf.d/host-1-stream.conf", baseline);
  ...
  it("still recognizes host-<id>.conf (HTTP) alongside the stream basename for the same host id", () => {
    const row = makeStreamHostRow({
      locations: [
        {
          path: "/",
    ...
    hostsStore.set(1, row);
    const httpBaseline = buildServerBlock(rowToHostConfig(row), new Map());

    const httpPreview = previewConfigEdit(auth, "conf.d/host-1.conf", httpBaseline);
    expect(httpPreview.hostId).toBe(1);
    expect(httpPreview.refusals).toEqual([]);

    const streamBaseline = streamBaselineText(row);
    const streamPreview = previewConfigEdit(auth, "conf.d/host-1-stream.conf", streamBaseline);
    expect(streamPreview.hostId).toBe(1);
    expect(streamPreview.refusals).toEqual([]);
  });

  it("throws ConfigClassificationError mentioning the draft for a stream file when the host has one", () => {
    hostsStore.set(1, makeStreamHostRow({ draft: { some: "change" } }));
    expect(() => applyConfigEdit(auth, "conf.d/host-1-stream.conf", "server {}")).toThrow(
      ConfigClassificationError
    );
    expect(writeFileSync).not.toHaveBeenCalled();
  });

  it("updates streamPorts in the DB and writes the regenerated stream file for a clean port change", () => {
    const row = makeStreamHostRow();
    hostsStore.set(1, row);
    const baseline = streamBaselineText(row);
    // Cosmetic whitespace difference (extra space) on an unrelated line, plus
    // the real value change — proves the write is regenerated, not the raw
    // user text, since diffAst treats "proxy_pass  x;"/"proxy_pass x;" identically.
    const actual = baseline
      .replace("listen 3306;", "listen 3307;")
      .replace("listen [::]:3306;", "listen [::]:3307;")
      .replace("proxy_pass stream_host_1_port_0;", "proxy_pass  stream_host_1_port_0;");

    const result = applyConfigEdit(auth, "conf.d/host-1-stream.conf", actual);

    expect(result.saved).toBe(true);
    expect(result.valid).toBe(true);
    expect(result.applied).toHaveLength(1);
    expect(result.applied[0]).toMatchObject({ kind: "stream-field", field: "port", to: 3307 });
    expect(hostsStore.get(1).streamPorts[0].port).toBe(3307);

    const writes = vi.mocked(writeFileSync).mock.calls;
    const finalWrite = writes[writes.length - 1];
    expect(finalWrite[0]).toBe("/data/nginx/conf.d/host-1-stream.conf");
    expect(finalWrite[1]).toContain("listen 3307;");
    expect(finalWrite[1]).not.toBe(actual);
  });

  it("rolls back streamPorts in the DB and restores the file when the regenerated stream config fails nginx -t", () => {
    const row = makeStreamHostRow();
    hostsStore.set(1, row);
    const baseline = streamBaselineText(row);
    const actual = baseline
    ...
    expect(() => applyConfigEdit(auth, "conf.d/host-1-stream.conf", actual)).toThrow(
      ConfigClassificationError
    );

    // DB rolled back
    expect(hostsStore.get(1).streamPorts[0].port).toBe(3306);
    // file restored to the original as the last write
    const writes = vi.mocked(writeFileSync).mock.calls;
    expect(writes[writes.length - 1]).toEqual(["/data/nginx/conf.d/host-1-stream.conf", baseline]);
    expect(reloadNginx).not.toHaveBeenCalled();
  });

  it("throws and writes nothing for a stream refusal (renaming a stream upstream)", () => {
    const row = makeStreamHostRow();
    hostsStore.set(1, row);
    const baseline = streamBaselineText(row);
    // Real upstream rename: swap "stream_host_1_port_0" for a different name
    // everywhere it's used (block header + proxy_pass), same shape as the
    // "removed+added" trap covered directly in classify.test.ts.
    const actual = baseline.replace(/stream_host_1_port_0/g, "stream_host_1_port_0_renamed");
    expect(actual).not.toBe(baseline);

    let caught: ConfigClassificationError | undefined;
    try {
      applyConfigEdit(auth, "conf.d/host-1-stream.conf", actual);
    } catch (err) {
      caught = err as ConfigClassificationError;
    }
    expect(caught).toBeInstanceOf(ConfigClassificationError);
  ...
  it("logs one audit row per stream edit, addressed as streamPorts[i].field", () => {
    const row = makeStreamHostRow();
    hostsStore.set(1, row);
    const baseline = streamBaselineText(row);
    const actual = baseline
    ...
    expect(hostAuditCalls[0][0]).toMatchObject({
      ...
      entity: "host",
      entityId: 1,
      details: expect.objectContaining({ field: "streamPorts[0].port", source: "config-editor" }),
    });
  });

  it("previewConfigEdit classifies a stream file without writing or mutating the DB", () => {
    const row = makeStreamHostRow();
    hostsStore.set(1, row);
    const baseline = streamBaselineText(row);
    const actual = baseline
    ...
    const preview = previewConfigEdit(auth, "conf.d/host-1-stream.conf", actual);
